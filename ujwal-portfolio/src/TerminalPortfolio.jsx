...
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
    ...
    AI Projects      ██████░░░░░░ Intermediate
    CEH Basics       ██████░░░░░░ Foundation
    APIs & Webhooks  ████████░░░░ Intermediate ||
    WhatsApp Bot Dev ██████░░░░░░ (LangChain / LangGraph)
...
🤖 Pro Tip: Mention this terminal portfolio
            in your message for nerd bonus points. 😉
      `,
    },
  };
...
• Type 'about', 'skills', 'projects', 'experience', or 'contact'
  ...
  // ─────────────────────────────────────────────
  // HELPERS: RENDER CONTENT WITH CLICKABLE LINKS
  // ─────────────────────────────────────────────
  const renderEntryContent = (content) => {
      ...
      if (part.match(/^https?:\/\//)) {
        return (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="underline text-emerald-300 hover:text-emerald-100"
          >
            {part}
          </a>
        );
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-b from-black via-zinc-950 to-black text-green-400 font-mono px-2 py-3 sm:p-4 flex items-center justify-center"
      onClick={() => inputRef.current?.focus()}
    >
      <div className="w-full max-w-5xl mx-auto">
        ...
        {showBanner && (
          <div className="mb-7">
            <div className="rounded-2xl border border-emerald-400 px-4 sm:px-6 py-3 sm:py-4 bg-[radial-gradient(circle_at_top_left,rgba(34,197,94,0.4),#020617)] shadow-[0_0_32px_rgba(34,197,94,1)] flex flex-col sm:flex-row items-stretch justify-between gap-4 sm:gap-6">
              {/* Left: title section */}
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-full border-2 border-emerald-400 flex items-center justify-center text-xl sm:text-2xl shadow-[0_0_14px_rgba(34,197,94,0.9)]">
                  💻
                </div>
                <div className="space-y-0.5 sm:space-y-1">
                  <p className="text-[0.7rem] sm:text-xs tracking-[0.18em] uppercase text-emerald-100/90">
                    Welcome, human
                  </p>
                  <p className="text-base sm:text-lg font-extrabold text-emerald-50">
                    UJWAL M&apos;s Terminal Portfolio
                  </p>
                  <p className="text-[0.7rem] sm:text-xs text-emerald-50/90">
                    Type <span className="font-semibold">help</span> or use the
                    quick commands below to explore.
                  </p>
                </div>
              </div>

              {/* Right: joke section */}
              <div className="sm:max-w-xs text-[0.78rem] sm:text-[0.85rem] text-lime-50">
                <p className="text-[0.68rem] sm:text-[0.72rem] uppercase tracking-[0.18em] text-lime-200 mb-1">
                  😂 Joke of this session
                </p>
                <div className="rounded-lg border border-lime-300/70 bg-emerald-950/90 px-2.5 py-2 leading-snug font-semibold">
                  {joke}
                </div>
                <p className="mt-1 text-[0.65rem] sm:text-[0.7rem] text-emerald-100/80">
                  (Refresh the page to get a new one 😉)
                </p>
              </div>
            </div>
        ...
        <div className="relative rounded-xl border border-green-500/30 bg-black/80 shadow-[0_0_30px_rgba(34,197,94,0.45)] overflow-hidden">
          {/* Top bar */}
          <div className="flex items-center justify-between px-3 sm:px-4 py-2 border-b border-green-500/30 bg-gradient-to-r from-zinc-900/80 to-black/80 text-[10px] sm:text-xs text-green-300/80">
            <div className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-400/70" />
              <span className="h-2.5 w-2.5 rounded-full bg-amber-300/60" />
              <span className="h-2.5 w-2.5 rounded-full bg-red-500/70" />
          ...
          <div
            ref={terminalRef}
            className="max-h-[70vh] min-h-[45vh] overflow-y-auto overflow-x-auto px-3 sm:px-4 py-3 text-[12px] sm:text-[13px] leading-relaxed"
          >
            {history.map((entry, index) => (
              <div key={index} className="mb-1">
                {entry.type === 'input' ? (
            ...
            {/* Input line */}
            <div className="flex items-center mt-1">
              <span className="text-emerald-400 mr-2 text-[12px] sm:text-[13px]">
                {getPrompt()}
              </span>
              <input
                ref={inputRef}
                ...
                onChange={(e) => setInput(e.target.value)}
                onKeyDown={handleKeyDown}
                className="flex-1 bg-transparent outline-none text-green-200 caret-green-400 text-[12px] sm:text-[13px]"
                autoFocus
                spellCheck="false"
              />
            </div>
          ...
          {/* Quick commands bar */}
          <div className="border-t border-green-500/20 bg-black/70 px-3 sm:px-4 py-2 text-[10px] sm:text-[11px] text-green-300/80 flex items-center gap-2 overflow-x-auto">
            <span className="mr-1 flex-shrink-0 text-green-400/70">
              Quick commands:
            </span>
            {quickCommands.map((cmd) => (
              <button
                ...
                type="button"
                onClick={() => handleCommand(cmd)}
                className="px-2 py-0.5 rounded border border-green-500/40 hover:bg-green-500/10 transition text-[10px] sm:text-[11px] flex-shrink-0"
              >
                {cmd}
              </button>
            ))}
